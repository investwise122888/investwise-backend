...
def fetch_weekly_data(symbol: str, weeks: int = 52):
    """Fetch weekly OHLCV data – try Stooq CSV first, fallback yfinance, then mock."""
    end = datetime.now()
    start = end - timedelta(weeks=weeks)
    
    # 1. Direct Stooq CSV fetch (no pandas_datareader)
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if not df.empty and 'Date' in df.columns:
                ...
                # Stooq returns data from earliest to latest, already ascending
...
def generate_mock_weekly(symbol: str, weeks: int):
    """Generate mock weekly data for development when all real sources fail."""
    end = datetime.now()
    start = end - timedelta(weeks=weeks)
