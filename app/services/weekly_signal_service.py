...
def fetch_weekly_data(symbol: str, weeks: int = 52):
    """Fetch weekly OHLCV data – try Stooq first, fallback yfinance, then mock."""
    end = datetime.now()
    start = end - timedelta(weeks=weeks)
    
    # 1. Try Stooq (using .PH suffix)
    try:
        df = pdr.get_data_stooq(f"{symbol}.PH", start, end)
        if not df.empty:
            df = df.sort_index(ascending=True)   # Stooq returns newest first
            logger.info(f"Stooq success for {symbol}")
            return df
...
def fetch_fundamentals(symbol):
    """Retrieve fundamentals document from Firestore, return dict or None."""
...
def generate_weekly_signal(symbol):
    """Compute weekly signal, apply fundamentals gate, return full result."""
