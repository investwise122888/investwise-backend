...
async def fetch_news_for_stock(company_name: str, symbol: str) -> List[Dict[str, Any]]:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            data = resp.json()
            articles = data.get("results", [])   # NewsData.io uses "results"
            results = []
            ...
            for article in articles[:5]:
                ...
                description = article.get("content", "") or article.get("description", "")  # content field
                if not description:
                    description = title
        ...
        print(f"NewsData.io failed for {symbol}: {e}. Returning mock.")
        return [
            {
