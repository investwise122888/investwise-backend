...
from app.services.ai_research_service import get_latest_ai_research, refresh_ai_research
...
async def get_ai_research(
    ...
    if not research:
        research = await refresh_ai_research()
        if not research:
            raise HTTPException(status_code=404, detail="No AI research data available.")
    return research
